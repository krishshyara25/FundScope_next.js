// src/app/customer/portfolio/page.jsx
'use client';
...
import { 
    ...
    Divider, TextField, Autocomplete, Snackbar, useTheme, Stack, IconButton, Tooltip, Chip
} from '@mui/material';
import Link from 'next/link';
import { 
    AccountBalanceWallet, TrendingUp, Timeline, Delete, Refresh, Money 
} from '@mui/icons-material';
import { CurrencyText } from '@/components/ReturnComponents'; 
...
async function fetchVirtualPortfolio() {
    const res = await fetch(`${BASE_URL}/api/v1/customer/portfolio`); // <-- UPDATED API PATH
    if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.error || 'Failed to fetch virtual portfolio');
    }
...
export default function CustomerPortfolioPage() { // <-- RENAMED COMPONENT
    const theme = useTheme(); 
    const [portfolio, setPortfolio] = useState([]);
    const [credit, setCredit] = useState({ currentBalance: 0, initialCredit: 0 }); 
    ...
    const isAccessAllowed = user && (user.role === REQUIRED_ROLE || user.role === 'seller' || user.role === 'admin' || user.role === 'company_head');
    // ----------------------
